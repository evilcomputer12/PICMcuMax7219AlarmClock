...
const uint8_t MAX7219_Dot_Matrix_font [256] [8] = {
  ...
  { 0x00, 0x7F, 0x0E, 0x1C, 0x0E, 0x7F, 0x00, 0x00 },
...
{
 max7219_cmd(0x09, 0);
 max7219_cmd(0x0a, 0x01);
 max7219_cmd(0x0B, 0x07);
...
void shiftchar (uint8_t ch, int delay)
{
 ...
 for (int i=0; i<8 -1; i++)
 {
  ...
  for (int j=7; j>=0; j--)
  {
   if ((MAX7219_Dot_Matrix_font[ch][indx])&(1<<j))
   {
    data |= (1<<(7-j));
   }
  }
...
void printString(uint8_t *str)
{
    ...
    while (str[strindx] != '\0')
    {
        ...
        for (int i = 0; i < 8; i++)
        {
            ...
            for (int j = 7; j >= 0; j--)
            {
                if ((MAX7219_Dot_Matrix_font[str[strindx]][indx]) & (1 << j))
                {
                    data |= (1 << (7 - j));
                }
            }
